...
from module import DistillationModule, Conv2d_BN, ResBlock
...
class RZYNet:
    ...
    def buildPRNet3(self):
        feature_size = 16
        inpt = Input(shape=(256, 256, 3,))
        x = Conv2d_BN(inpt, nb_filter=feature_size, kernel_size=3, strides=(1, 1), padding='same')  # 256 256 16
        # x = Conv2D(filter=size, kernel_size=, padding='same', strides=(1,1), activation='relu', name='Conv0')(inpt)
        x = ResBlock(x, nb_filter=feature_size * 2, kernel_size=3, strides=(2, 2),
                     with_conv_shortcut=True)  # 128 128 32
        x = ResBlock(x, nb_filter=feature_size * 2, kernel_size=3, strides=(1, 1),
                     with_conv_shortcut=False)  # 128 128 32
        x = ResBlock(x, nb_filter=feature_size * 4, kernel_size=3, strides=(2, 2), with_conv_shortcut=True)  # 64 64 64
        x = ResBlock(x, nb_filter=feature_size * 4, kernel_size=3, strides=(1, 1), with_conv_shortcut=False)  # 64 64 64

        x = ResBlock(x, nb_filter=feature_size * 8, kernel_size=3, strides=(2, 2), with_conv_shortcut=True)  # 32 32 128
        x = ResBlock(x, nb_filter=feature_size * 8, kernel_size=3, strides=(1, 1),
                     with_conv_shortcut=False)  # 32 32 128
        x = ResBlock(x, nb_filter=feature_size * 16, kernel_size=3, strides=(2, 2),
                     with_conv_shortcut=True)  # 16 16 256
        x = ResBlock(x, nb_filter=feature_size * 16, kernel_size=3, strides=(1, 1),
                     with_conv_shortcut=False)  # 16 16 256
        x = ResBlock(x, nb_filter=feature_size * 32, kernel_size=3, strides=(2, 2), with_conv_shortcut=True)  # 8 8 512
        x = ResBlock(x, nb_filter=feature_size * 32, kernel_size=3, strides=(1, 1), with_conv_shortcut=False)  # 8 8 512

        x = Conv2DTranspose(filters=feature_size * 32, kernel_size=3, strides=(1, 1), activation='relu',
                            padding='same')(
            x)  # 8 8 512
        x = Conv2DTranspose(filters=feature_size * 16, kernel_size=3, strides=(2, 2), activation='relu',
                            padding='same')(
            x)  # 16 16 256
        x = Conv2DTranspose(filters=feature_size * 16, kernel_size=3, strides=(1, 1), activation='relu',
                            padding='same')(
            x)  # 16 16 256
        x = Conv2DTranspose(filters=feature_size * 16, kernel_size=3, strides=(1, 1), activation='relu',
                            padding='same')(
            x)  # 16 16 256
        x = Conv2DTranspose(filters=feature_size * 8, kernel_size=3, strides=(2, 2), activation='relu', padding='same')(
            x)  # 32 32 128
        x = Conv2DTranspose(filters=feature_size * 8, kernel_size=3, strides=(1, 1), activation='relu', padding='same')(
            x)  # 32 32 128
        x = Conv2DTranspose(filters=feature_size * 8, kernel_size=3, strides=(1, 1), activation='relu', padding='same')(
            x)  # 32 32 128
        x = Conv2DTranspose(filters=feature_size * 4, kernel_size=3, strides=(2, 2), activation='relu', padding='same')(
            x)  # 64 64 64
        x = Conv2DTranspose(filters=feature_size * 4, kernel_size=3, strides=(1, 1), activation='relu', padding='same')(
            x)  # 64 64 64
        x = Conv2DTranspose(filters=feature_size * 4, kernel_size=3, strides=(1, 1), activation='relu', padding='same')(
            x)  # 64 64 64

        x = Conv2DTranspose(filters=feature_size * 2, kernel_size=3, strides=(2, 2), activation='relu', padding='same')(
            x)  # 128 128 32
        x = Conv2DTranspose(filters=feature_size * 2, kernel_size=3, strides=(1, 1), activation='relu', padding='same')(
            x)  # 128 128 32
        x = Conv2DTranspose(filters=feature_size, kernel_size=3, strides=(2, 2), activation='relu', padding='same')(
            x)  # 256 256 16
        x = Conv2DTranspose(filters=feature_size, kernel_size=3, strides=(1, 1), activation='relu', padding='same')(
            x)  # 256 256 16

        x = Conv2DTranspose(filters=3, kernel_size=3, strides=(1, 1), activation='relu', padding='same')(x)  # 256 256 3
        x = Conv2DTranspose(filters=3, kernel_size=3, strides=(1, 1), activation='relu', padding='same')(x)  # 256 256 3
        x = Conv2DTranspose(filters=3, kernel_size=3, strides=(1, 1), activation='sigmoid', padding='same')(
            x)  # 256 256 3
