...
class RZYNet:
    ...
    def compileOffsetModel(self):
        ...
        if self.gpu_num > 1:
            para_model = multi_gpu_model(model, gpus=self.gpu_num)
            para_model.compile(loss={'Posmap': getLossFunction(self.loss_function, rate=0.01),
                                     'Offset': getLossFunction(self.loss_function, rate=1),
                                     'ParamR': getLossFunction('mae', rate=0.1),
                                     'ParamT': getLossFunction('mae', rate=0.1),
            ...
            model.compile(loss={'Posmap': getLossFunction(self.loss_function, rate=0.1),
                                'Offset': getLossFunction(self.loss_function, rate=10),
                                'ParamR': getLossFunction('mae', rate=1), 'ParamT': getLossFunction('mae', rate=1),
                                'ParamS': getLossFunction('mae', rate=1)},
                          optimizer=self.optimizer,
                          metrics={'Posmap': getLossFunction('frse'), 'Offset': getLossFunction('frse'),
                                   'ParamR': 'mae', 'ParamT': 'mae', 'ParamS': 'mae'})
            model.summary()
    ...
    def buildInitPRNet(self):
        ...
        # x = Conv2D(filter=size, kernel_size=, padding='same', strides=(1,1), activation='relu', name='Conv0')(inpt)
        x = PRNResBlock(x, filters=feature_size * 2, kernel_size=4, strides=(2, 2),
                        with_conv_shortcut=True)  # 128 128 32
        x = PRNResBlock(x, filters=feature_size * 2, kernel_size=4, strides=(1, 1),
                        with_conv_shortcut=False)  # 128 128 32
    ...
    def buildCbamPRNet(self):
        ...
        # x = Conv2D(filter=size, kernel_size=, padding='same', strides=(1,1), activation='relu', name='Conv0')(inpt)
        x = CbamResBlock(x, nb_filter=feature_size * 2, kernel_size=4, strides=(2, 2),
                         with_conv_shortcut=True)  # 128 128 32
        x = CbamResBlock(x, nb_filter=feature_size * 2, kernel_size=4, strides=(1, 1),
                         with_conv_shortcut=False)  # 128 128 32
        x = CbamResBlock(x, nb_filter=feature_size * 4, kernel_size=4, strides=(2, 2),
                         with_conv_shortcut=True)  # 64 64 64
        x = CbamResBlock(x, nb_filter=feature_size * 4, kernel_size=4, strides=(1, 1),
                         with_conv_shortcut=False)  # 64 64 64

        x = CbamResBlock(x, nb_filter=feature_size * 8, kernel_size=4, strides=(2, 2),
                         with_conv_shortcut=True)  # 32 32 128
        x = CbamResBlock(x, nb_filter=feature_size * 8, kernel_size=4, strides=(1, 1),
                         with_conv_shortcut=False)  # 32 32 128
        x = CbamResBlock(x, nb_filter=feature_size * 16, kernel_size=4, strides=(2, 2),
                         with_conv_shortcut=True)  # 16 16 256
        x = CbamResBlock(x, nb_filter=feature_size * 16, kernel_size=4, strides=(1, 1),
                         with_conv_shortcut=False)  # 16 16 256
        x = CbamResBlock(x, nb_filter=feature_size * 32, kernel_size=4, strides=(2, 2),
                         with_conv_shortcut=True)  # 8 8 512
        x = CbamResBlock(x, nb_filter=feature_size * 32, kernel_size=4, strides=(1, 1),
                         with_conv_shortcut=False)  # 8 8 512
    ...
    def buildOffsetPRNet(self):
        ...
        x = PRNResBlock(x, filters=feature_size * 16, kernel_size=4, strides=(1, 1),
                        with_conv_shortcut=False)  # 16 16 256
        x = PRNResBlock(x, filters=feature_size * 32, kernel_size=4, strides=(2, 2), with_conv_shortcut=True)  # 8 8 512
        fs = PRNResBlock(x, filters=feature_size * 32, kernel_size=4, strides=(1, 1),
                         with_conv_shortcut=False)  # 8 8 512
        ...
        offset = Conv2d_Transpose_BN_AC(x, filters=3, kernel_size=4, strides=(1, 1), activation='tanh', padding='same',
                                        name='Offset')  # 256 256 3
